                        ...
                        <tbody>
                            ...
                            @php
                                ...
                                if($order->payment_status == 'paid'){
                                    $status = '<span class="bg-green-600 py-1 px-3 rounded text-white shadow">Paid/span>';
                                }

                                if($order->payment_status == 'failed'){
                                    $status = '<span class="bg-red-600 py-1 px-3 rounded text-white shadow">Failed</span>';
                                }
                            @endphp
                                <tr class="odd:bg-white even:bg-gray-100 dark:odd:bg-slate-900 dark:even:bg-slate-800" wire:key="{{ $order->id }}">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-gray-200" {!! $status !!}>
                                        <span class="bg-orange-500 py-1 px-3 rounded text-white shadow">{{ $order->status }}/span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-gray-200">
                                        {!! $payment_status !!}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-gray-200">
                                        {{ Number::currency($order->grand_total,'NPR') }}</td>
